...
@Injectable()
export class ApExamStorageService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.region =
      this.getFirstNonEmptyConfig("AWS_REGION", "AWS_DEFAULT_REGION") || "ap-south-1";
    this.publicBaseUrl = this.getFirstNonEmptyConfig("AWS_URL");
    const usePathStyleRaw =
      this.getFirstNonEmptyConfig("AWS_USE_PATH_STYLE_ENDPOINT") || "false";
  ...
  async uploadReport(file: UploadedReportFile, registrationId: string) {
    if (!this.bucketName) {
    ...
    const key = `ap-exam/reports/${registrationId}/${Date.now()}-${this.sanitizeFileName(file.originalname)}.${extension}`;
